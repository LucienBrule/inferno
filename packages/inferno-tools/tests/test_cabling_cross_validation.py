...
def sample_topology():
    ...
    return {
        'racks': [
            {'rack_id': 'rack-1', 'uplinks_qsfp28': 2},
            {'rack_id': 'rack-2', 'uplinks_qsfp28': 2}
        ],
        'wan': {'uplinks_cat6a': 2}
    }
...
def sample_tors():
    ...
    return {
        'tors': [
            {'id': 'tor-1', 'rack_id': 'rack-1'},
            {'id': 'tor-2', 'rack_id': 'rack-2'}
        ],
        'spine': {'id': 'spine-1'}
    }
...
class TestCrossValidation:
    ...
    def test_normalize_bom_old_format(self):
        ...
        old_bom = {
            'sfp28_25g_dac': {3: 8},
            'qsfp28_100g_aoc': {7: 4},
            'rj45_cat6a': {7: 15}  # High count -> mgmt
        }
    ...
    def test_aggregate_intent_to_class_structure(self, sample_topology, sample_nodes, sample_tors, sample_site, sample_policy):
    ...
    def test_reconcile_happy_path(self, sample_policy):
        ...
        bom_structure = {
            'leaf-node': {'sfp28_25g': {3: 8}},
            'leaf-spine': {'qsfp28_100g': {7: 4}}
        }
        intent_structure = {
            'leaf-node': {'sfp28_25g': {3: 8}},
            'leaf-spine': {'qsfp28_100g': {7: 4}}
    ...
    def test_reconcile_missing_links(self, sample_policy):
        ...
        bom_structure = {
            'leaf-node': {'sfp28_25g': {3: 6}}  # Missing 2
        }
        intent_structure = {
            'leaf-node': {'sfp28_25g': {3: 8}}
        }
    ...
    def test_reconcile_phantom_items(self, sample_policy):
        ...
        bom_structure = {
            'leaf-spine': {'qsfp28_100g': {7: 6}}  # Extra 2
        }
        intent_structure = {
            'leaf-spine': {'qsfp28_100g': {7: 4}}
        }
    ...
    def test_reconcile_bin_mismatch_warn(self, sample_policy):
        ...
        bom_structure = {
            'leaf-node': {'sfp28_25g': {5: 8}}  # 5m instead of 3m (within 2m slop)
        }
        intent_structure = {
            'leaf-node': {'sfp28_25g': {3: 8}}
        }
    ...
    def test_reconcile_bin_mismatch_fail(self, sample_policy):
        ...
        bom_structure = {
            'leaf-node': {'sfp28_25g': {1: 8}}  # 1m instead of 3m (too small)
        }
        intent_structure = {
            'leaf-node': {'sfp28_25g': {3: 8}}
        }
...
class TestCrossValidationIntegration:
    ...
    def test_cross_validate_bom_happy_path(self, mock_policy, mock_site, mock_nodes, mock_tors, mock_topology, 
                                         sample_topology, sample_tors, sample_nodes, sample_site, sample_policy, happy_bom):
    ...
    @patch('inferno_tools.cabling.cross_validate.load_site')
    @patch('inferno_tools.cabling.cross_validate.load_cabling_policy')
    def test_cross_validate_bom_missing_links(self, mock_policy, mock_site, mock_nodes, mock_tors, mock_topology,
                                            sample_topology, sample_tors, sample_nodes, sample_site, sample_policy, missing_leaf_node_bom):
    ...
    @patch('inferno_tools.cabling.cross_validate.load_site')
    @patch('inferno_tools.cabling.cross_validate.load_cabling_policy')
    def test_cross_validate_bom_phantom_items(self, mock_policy, mock_site, mock_nodes, mock_tors, mock_topology,
                                            sample_topology, sample_tors, sample_nodes, sample_site, sample_policy, phantom_leaf_spine_bom):
