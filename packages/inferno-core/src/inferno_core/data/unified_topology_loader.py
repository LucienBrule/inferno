...
from inferno_core.models.unified_topology import (
    UnifiedTopology, UnifiedSwitch, UnifiedInterface, UnifiedPorts,
    UnifiedRack, UnifiedWan
)
...
def _derive_capacity_info(spines: List[UnifiedSwitch], leafs: List[UnifiedSwitch]) -> tuple[UnifiedSwitch, List[UnifiedRack], UnifiedWan]:
    """
            ...
            if "interfaces" in spine_data:
                for iface_data in spine_data["interfaces"]:
                    interfaces.append(UnifiedInterface(
                        ...
                        connects_to=iface_data.get("connects_to")
                    ))
...
def load_unified_topology_from_capacity_format(path: Path | str) -> UnifiedTopology:
    ...
    try:
        ...
        spine_ports = UnifiedPorts(
            qsfp28_total=spine_data.get("ports", {}).get("qsfp28_total", 32)
        )
        spine = UnifiedSwitch(
            id=spine_data["id"],
            model=spine_data["model"],
        ...
        for rack_data in racks_data:
            racks.append(UnifiedRack(
                ...
                tor_id=rack_data["tor_id"],
                uplinks_qsfp28=rack_data["uplinks_qsfp28"]
            ))
