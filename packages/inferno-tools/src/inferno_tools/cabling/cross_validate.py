...
from inferno_core.data.network_loader import (
    load_topology, load_tors, load_nodes, load_site
)
...
def _derive_intent_links(topology, tors, nodes, site, policy) -> List[Dict[str, Any]]:
    ...
    for node in nodes.get('nodes', []):
        ...
        dac_max_m = sfp28_rules.get('dac_max_m', 3.0)
        bins = sfp28_rules.get('bins_m', [1, 2, 3, 5, 7, 10])
        
        cable_type = 'sfp28_25g_dac' if distance <= dac_max_m else 'sfp28_25g_aoc'
    ...
    for rack in topology.get('racks', []):
        ...
        dac_max_m = qsfp28_rules.get('dac_max_m', 3.0)
        bins = qsfp28_rules.get('bins_m', [1, 2, 3, 5, 7, 10])
        
        cable_type = 'qsfp28_100g_dac' if distance <= dac_max_m else 'qsfp28_100g_aoc'
        length_bin = _select_length_bin(distance, bins)
...
def cross_validate_bom(
    ...
    # Derive intent links
    intent_links = _derive_intent_links(topology.model_dump(), tors.model_dump(), nodes.model_dump(), 
                                       site.model_dump() if site else None, policy)
