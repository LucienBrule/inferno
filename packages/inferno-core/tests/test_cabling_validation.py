...
from inferno_core.validation.cabling import (
    run_cabling_validation, validate_ports, validate_compatibility,
    validate_oversubscription, validate_completeness, validate_lengths,
    validate_redundancy, validate_policy_sanity, Finding, Report
)
from inferno_core.data.network_loader import (
    NodeRec, TorRec, TopologyRec, SiteRec, NicRec, TorPorts, SpinePorts,
    SpineRec, TopologyRackRec, TopologyWanRec, SiteRackRec
)
...
def sample_nodes():
    ...
    return [
        NodeRec(
            ...
            nics=[NicRec(type="SFP28", count=2)]
        ),
        NodeRec(
            id="node-2",
            rack_id="rack-1",
...
def sample_topology():
    ...
    return TopologyRec(
        spine=SpineRec(
            id="spine-1",
            model="spine-switch",
            ports=SpinePorts(qsfp28_total=32)
        ),
        racks=[
...
class TestOversubscriptionValidation:
    ...
    def test_oversubscription_warning(self, sample_topology, sample_tors, sample_nodes, sample_policy):
        ...
        limited_topology = TopologyRec(
            ...
            racks=[
                TopologyRackRec(rack_id="rack-1", tor_id="tor-1", uplinks_qsfp28=1)  # Only 100 Gbps uplink
            ],
            wan=TopologyWanRec(uplinks_cat6a=2)
        )
    ...
    def test_oversubscription_critical_failure(self, sample_topology, sample_tors, sample_nodes, sample_policy):
        ...
        findings = validate_oversubscription(very_limited_topology, sample_tors, very_high_bandwidth_nodes, sample_policy)
    ...
    def test_no_uplinks_failure(self, sample_topology, sample_tors, sample_nodes, sample_policy):
        ...
        no_uplinks_topology = TopologyRec(
            spine=SpineRec(id="spine-1", model="spine-switch", ports=SpinePorts(qsfp28_total=32)),
            racks=[
                TopologyRackRec(rack_id="rack-1", tor_id="tor-1", uplinks_qsfp28=0)
            ],
            wan=TopologyWanRec(uplinks_cat6a=2)
        )
...
class TestCompletenessValidation:
    ...
    def test_missing_tor_mapping(self, sample_topology, sample_tors, sample_nodes, sample_policy):
        ...
        bad_topology = TopologyRec(
            spine=SpineRec(id="spine-1", model="spine-switch", ports=SpinePorts(qsfp28_total=32)),
            racks=[
                TopologyRackRec(rack_id="rack-1", tor_id="unknown-tor", uplinks_qsfp28=4)
            ],
            wan=TopologyWanRec(uplinks_cat6a=2)
        )
...
class TestLengthValidation:
    ...
    def test_length_exceeds_dac_with_aoc_available(self, sample_topology, sample_tors, sample_nodes, sample_site, sample_policy):
...
class TestRedundancyValidation:
    ...
    def test_dual_homing_odd_nics_failure(self, sample_topology, sample_tors, sample_policy):
        ...
        odd_nic_nodes = [
            NodeRec(id="node-1", rack_id="rack-1", nics=[NicRec(type="SFP28", count=3)])
        ]
...
class TestNicCompatibility:
    ...
    def test_sfp28_no_compatible_ports(self, sample_topology, sample_nodes, sample_policy):
        ...
        no_sfp28_tors = {
            "tor-1": TorRec(
                id="tor-1",
                rack_id="rack-1",
                model="qsfp-only-switch",
                ports=TorPorts(sfp28_total=0, qsfp28_total=8)
            )
        }
    ...
    def test_qsfp28_nic_unsupported(self, sample_topology, sample_tors, sample_policy):
        ...
        qsfp28_nodes = [
            NodeRec(id="node-1", rack_id="rack-1", nics=[NicRec(type="QSFP28", count=1)])
        ]
    ...
    def test_rj45_unmodeled_warning(self, sample_topology, sample_tors, sample_policy):
        ...
        # Node with RJ45 NIC
        rj45_nodes = [
            NodeRec(id="node-1", rack_id="rack-1", nics=[NicRec(type="RJ45", count=1)])
        ]
...
class TestIntegration:
    ...
    def test_run_cabling_validation_success(self, mock_load_site, mock_load_nodes, 
                                          mock_load_tors, mock_load_topology,
                                          sample_topology, sample_tors, sample_nodes, sample_site):
    ...
    def test_report_serialization(self, sample_topology, sample_tors, sample_nodes, sample_policy):
        ...
        findings = validate_ports(sample_topology, sample_tors, sample_nodes, sample_policy)
        report = Report(
            summary={"pass": 10, "warn": 1, "fail": 0, "info": 1},
            findings=findings
        )
